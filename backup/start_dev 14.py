...
def ensure_frontend_ready() -> str:
    ...
    # Vite can fail with missing dep-*.js chunks when node_modules is reused after ZIP replacement
    # or when a previous install was interrupted. Verify the package, and force a clean reinstall if broken.
    needs_install = not vite_bin.exists()
    ...
    if vite_bin.exists():
        try:
            ...
            print("Frontend dependency check failed. Reinstalling node_modules...")
    ...
    if needs_install:
        ...
        try:
            run_checked([npm, "cache", "verify"], cwd=FRONTEND, env=npm_env, timeout=120)
...
def main():
    print("1Resource v1.0.24 - Local Startup")
