...
function ComboInput({ label, value, onChange, options = [], placeholder = 'Select or type new value' }) {
  ...
  const cleanOptions = [...new Set((options || []).map(v => String(v || '').trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  return <label>{label}<input list={listId} value={value ?? ''} placeholder={placeholder} onChange={e => onChange(e.target.value)} /><datalist id={listId}>{cleanOptions.map(o => <option key={o} value={o} />)}</datalist></label>;
}
...
function DemandPage({ demand, filters, setFilters, onCreate, onSelect, onEdit }) {
  return <section className="pageBlock"><div className="sectionHeader"><div><h2>Demand Requests</h2><p>Demand-side role repository used to match Truflux supply against client needs and rate-card targets.</p></div><button className="primary" onClick={onCreate}>Add demand</button></div>
    ...
    <div className="hintBox comboHint"><strong>Demand row click</strong><span>Click any demand row to edit it. Use View for shortlist, MCQ and match details.</span></div>
    <div className="tableWrap"><table><thead><tr><th>Demand</th><th>Client / Project</th><th>Role</th><th>Skills</th><th>Positions</th><th>Rate Target</th><th>Priority</th><th>Status</th><th>View</th></tr></thead><tbody>{demand.map(d => <tr key={d.id} className="clickableRow" onClick={() => onEdit(d)} title="Click to edit demand"><td>{d.demand_code}</td><td><strong>{d.client_name || '-'}</strong><br /><small>{d.project_name}</small></td><td>{d.role_title}<br /><small>{d.domain}</small></td><td>{d.required_skills}</td><td>{d.number_of_positions}</td><td>{money(d.target_customer_rate)}<br /><small>Max cost {money(d.max_internal_cost)}</small></td><td><span className="pill">{d.priority}</span></td><td>{d.status}</td><td><button type="button" onClick={(e) => { e.stopPropagation(); onSelect(d.id); }}>View</button></td></tr>)}</tbody></table>{demand.length === 0 && <p className="empty">No demand requests found.</p>}</div>
  </section>;
}

function DemandForm({ initial, onCancel, onSave, demandOptions = [] }) {
  const [form, setForm] = useState(initial || emptyDemand);
  const isEdit = Boolean(form?.id);
  const clientOptions = useMemo(() => [...new Set((demandOptions || []).map(d => d.client_name).filter(Boolean))], [demandOptions]);
  const projectOptions = useMemo(() => {
    const selectedClient = String(form.client_name || '').trim().toLowerCase();
    const rows = selectedClient ? (demandOptions || []).filter(d => String(d.client_name || '').trim().toLowerCase() === selectedClient) : (demandOptions || []);
    return [...new Set(rows.map(d => d.project_name).filter(Boolean))];
  }, [demandOptions, form.client_name]);
  const clientSummary = [
  ...
  return <form className="candidateForm" onSubmit={e => { e.preventDefault(); onSave(form); }}>
    ...
    {!isEdit ? <><div className="hintBox comboHint"><strong>Client / Project selection</strong><span>Select an existing client or project from the dropdown, or type a new value to add it to future lists.</span></div><div className="formGrid">
      <ComboInput label="Client name" value={form.client_name} options={clientOptions} onChange={v => setForm({ ...form, client_name: v })} />
      <ComboInput label="Project name" value={form.project_name} options={projectOptions} onChange={v => setForm({ ...form, project_name: v })} />
      <Input label="Role title" value={form.role_title} onChange={v => setForm({ ...form, role_title: v })} />
...
1Resource Team`
  ...
  async function testEmailProvider() {
    ...
    try {
      ...
      setLocalMsg(res.message || (res.ok ? 'Email provider connection test passed.' : 'Email provider connection test failed.'));
    } catch (err) {
      setLocalMsg(`Email provider test failed: ${err.message}`);
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="outreachRight">
        <div className="panel compact stickyComposer">
          ...
          <div className="hintBox smtpStatusBox">
            ...
            <span>{smtpStatus?.configured ? `${smtpStatus.provider === 'mailjet_api' ? 'Mailjet API' : 'SMTP'} configured · From ${smtpStatus.from_email || '-'}${smtpStatus.host ? ` · ${smtpStatus.host}:${smtpStatus.port}` : ''}` : `${smtpStatus?.provider === 'mailjet_api' ? 'Mailjet API' : 'SMTP'} not configured. Emails will be logged as queued until provider variables are added on Railway.`}</span>
          </div>
          <div className="formActions"><button type="button" disabled={busy} onClick={testEmailProvider}>Test Email Provider</button><button className="primary" type="button" disabled={busy || (!selectedClients.length && !selectedUsers.length)} onClick={sendEmail}>Send / Queue Email</button></div>
        </div>
...
function App() {
  ...
  async function loadDashboard() { if (!token) return null; const data = await api(`/api/dashboard?_=${Date.now()}`, {}, token); setDashboard({ ...data }); return data; }
  ...
  async function loadCandidates() { if (!token) return []; const params = new URLSearchParams(filters).toString(); const data = await api(`/api/candidates?${params}&_=${Date.now()}`, {}, token); setCandidates(data); return data; }
  async function loadDemand() { if (!token) return []; const params = new URLSearchParams(demandFilters).toString(); const data = await api(`/api/demand?${params}&_=${Date.now()}`, {}, token); setDemand(data); return data; }
  ...
  async function refreshAfterMutation(message = 'Saved and dashboard refreshed') {
  ...
  async function saveCandidate(form) { const method = form.id ? 'PUT' : 'POST'; const path = form.id ? `/api/candidates/${form.id}` : '/api/candidates'; const saved = await api(path, { method, body: JSON.stringify(form) }, token); setEditing(null); setSelected(saved); await refreshAfterMutation('Candidate saved and dashboard numbers refreshed'); }
