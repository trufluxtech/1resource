...
function App() {
  ...
  async function loadDashboard() { if (token) setDashboard(await api(`/api/dashboard?_=${Date.now()}`, {}, token)); }
  ...
  async function saveCandidate(form) { const method = form.id ? 'PUT' : 'POST'; const path = form.id ? `/api/candidates/${form.id}` : '/api/candidates'; const saved = await api(path, { method, body: JSON.stringify(form) }, token); setEditing(null); setSelected(saved); await refreshWorkspace(false); show('Candidate saved and dashboard refreshed'); }
  async function deleteCandidate() { if (!selected || !confirm('Delete this candidate?')) return; await api(`/api/candidates/${selected.id}`, { method: 'DELETE' }, token); setSelected(null); await refreshWorkspace(false); show('Candidate deleted and dashboard refreshed'); }
  ...
  async function saveDemand(form) { const method = form.id ? 'PUT' : 'POST'; const path = form.id ? `/api/demand/${form.id}` : '/api/demand'; const saved = await api(path, { method, body: JSON.stringify(form) }, token); setEditingDemand(null); await refreshWorkspace(false); await loadSelectedDemand(saved.id); show('Demand saved and dashboard refreshed'); }
  async function deleteDemand() { if (!selectedDemand || !confirm('Delete this demand request?')) return; await api(`/api/demand/${selectedDemand.id}`, { method: 'DELETE' }, token); setSelectedDemand(null); await refreshWorkspace(false); show('Demand deleted and dashboard refreshed'); }
  async function shortlist(demandId, candidateId) { await api(`/api/demand/${demandId}/shortlist/${candidateId}`, { method: 'POST' }, token); if (selectedDemand?.id === demandId) await loadSelectedDemand(demandId); await refreshWorkspace(false); show('Candidate shortlisted and dashboard refreshed'); }
